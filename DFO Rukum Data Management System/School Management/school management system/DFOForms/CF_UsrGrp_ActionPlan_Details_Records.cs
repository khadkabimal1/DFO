    ...
    public partial class CF_UsrGrp_ActionPlan_Details_Records : Form
    {
        ...
        public void GetData()
        {
            try
            {
                ...
                con.Open();
                cmd = new SqlCommand("SELECT  CFUGACDtls,rtrim(IllakaName) [IllakaName],rtrim(FileNo) [FileNo],rtrim(CFName) [CFName],rtrim(VDC_MP_Name) [VDC_MP_Name],rtrim(LastRenewDate) [LastRenewDate],rtrim(CFCodeNo) [CFCodeNo] ,rtrim(KhandaCount) [KhandaCount], rtrim(HouldHold) [HouldHold],rtrim(Area) [Area] ,rtrim(CFCodeNo) [CFCodeNo],rtrim(MainSpecies) [MainSpecies] from tbl_CFUsrGrp_ActionPlan_Details ", con);
