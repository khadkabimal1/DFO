    ...
    public partial class CF_Forest_UserGroup_Constitution_DetailsRecords : Form
    {
        ...
        public void GetData()
        {
            try
            {
                ...
                con.Open();
                cmd = new SqlCommand("SELECT  CFUsrGrpCDtl,rtrim(IllakaName) [IllakaName],rtrim(CFName) [CFName],rtrim(VDC_MP_Name) [VDC_MP_Name] ,rtrim(HhDalit) [HhDalit],rtrim(HhJanjati) [HhJanjati],rtrim(HhOthersCaste) [HhOthersCaste] ,rtrim(HhTotal) [HhTotal],rtrim(HhTotal) [HhTotal] ,rtrim(SDalitM) [SDalitM],rtrim(SDalitF) [SDalitF] ,rtrim(SJanjatiM) [SJanjatiM],rtrim(SJanjatiF) [SJanjatiF] ,rtrim(SOthersM) [SOthersM],rtrim(SOthersF) [SOthersF],rtrim(STotalM) [STotalM] ,rtrim(STotalF) [STotalF],rtrim(GrandTotal) [GrandTotal] ,rtrim(BA) [BA],rtrim(BB) [BB] ,rtrim(BC) [BC],rtrim(BD) [BD]  from tbl_CFUsrGrpConstitutionDtls ", con);
